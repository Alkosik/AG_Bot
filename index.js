...
client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (
    newState.channelId === "790343982877900820" &&
    newState.id != "883767138433765386" &&
    ...
    console.log(
      chalk.green("VOICE INFO"),
      "Succesfully connected to voice channel."
    );
    ...
    voiceConnection.on(VoiceConnectionStatus.Ready, () => {
      console.log(
        chalk.green("VOICE INFO"),
        "The connection has entered the Ready state."
      );
    ...
    currently_playing == true &&
    oldState.channelId === "790343982877900820" &&
...
client.on("messageCreate", async (message) => {
  ...
  (async () => {
    if (
      ...
      // }
      console.log(
        chalk.yellow("DETECTION"),
        "Invite detected. Banning disabled"
      );
      let sentwarnmsg;
      const warnmsg = new EmbedBuilder()
        .setAuthor({
          name: "Gang Słoni",
          iconURL: "https://i.ibb.co/rk0Z6Mb/Grupfdgggdrszga-1.png",
        })
      ...
      message
        .delete()
        .then(
          (sentwarnmsg = await message.channel.send({ embeds: [warnmsg] }))
        );
      await snooze(5000);
      sentwarnmsg.delete().catch((error) => {
        ...
        if (error.code !== 10008) {
          console.error(
            chalk.red("ERROR"),
            "Failed to delete the warn message during link removal: ",
            error
          );
        }
      });
  ...
  // Other
  if (
    message.content.toLowerCase().startsWith("she") &&
    message.content.toLowerCase().endsWith("esh") &&
    !message.author.bot
  ) {
  ...
  if (message.content.toLowerCase().includes("plugg") && !message.author.bot) {
    message.reply("Plugg!");
  }
});
